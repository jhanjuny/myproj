...
def dimer_hoppings(t: float, delta_t: float) -> np.ndarray:
    return np.array([t + delta_t, t, t], dtype=float)


def bond_amplitude(k_point: np.ndarray, hoppings: np.ndarray, deltas: np.ndarray) -> complex:
    phases = np.exp(1j * (deltas @ k_point))
    return np.sum(hoppings * phases)


def bands_from_hoppings(k_point: np.ndarray, hoppings: np.ndarray, deltas: np.ndarray) -> np.ndarray:
    amplitude = abs(bond_amplitude(k_point, hoppings, deltas))
    return np.array([-amplitude, amplitude], dtype=float)


def pristine_bands(k_point: np.ndarray, t: float, deltas: np.ndarray) -> np.ndarray:
    return bands_from_hoppings(k_point, np.array([t, t, t], dtype=float), deltas)


def dimerized_bands(k_point: np.ndarray, t: float, delta_t: float, deltas: np.ndarray) -> np.ndarray:
    return bands_from_hoppings(k_point, dimer_hoppings(t, delta_t), deltas)


def compute_band_path(band_fn, k_path: list[tuple[str, np.ndarray]], samples_per_segment: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
...
def compute_dos(band_fn, b1: np.ndarray, b2: np.ndarray, grid_n: int, energy_axis: np.ndarray, broadening: float) -> np.ndarray:
...
def compute_reciprocal_map(band_fn, polygon: np.ndarray, grid_n: int) -> list[tuple[float, float, float, float]]:
...
def write_reciprocal_svg(
    ...
    lower_min = -float(energy_limit)
    lower_max = 0.0
    upper_min = 0.0
    upper_max = float(energy_limit)
...
<text x="{left_panel_x + panel_width + 14:.2f}" y="{panel_y + 18:.2f}" font-family="Consolas, monospace" font-size="12">E in [-{energy_limit:.2f}, 0]</text>
<text x="{right_panel_x + panel_width + 14:.2f}" y="{panel_y + 18:.2f}" font-family="Consolas, monospace" font-size="12">E in [0, {energy_limit:.2f}]</text>
</svg>
"""
...
def generate_real_space_patch(nx: int, ny: int) -> tuple[list[list[float]], list[str], list[float], list[tuple[int, int, float, str]], list[str], list[list[float]]]:
    ...
    used_pairs = set()
    for left_index, right_index in strong_pairs:
        bonds.append((left_index, right_index, 0.09, "#f28e2b"))
        used_pairs.add((left_index, right_index))

    for left_index in range(len(array_positions)):
        for right_index in range(left_index + 1, len(array_positions)):
            if (left_index, right_index) in used_pairs:
                continue
            distance = np.linalg.norm(array_positions[left_index] - array_positions[right_index])
            if 0.75 <= distance <= 1.05:
                bonds.append((left_index, right_index, 0.055, "#666666"))

    min_x = float(array_positions[:, 0].min())
    max_y = float(array_positions[:, 1].max())
    labels = ["A sublattice", "B sublattice", "Dimerized bond"]
    label_positions = [
        [min_x - 1.5, max_y + 0.8, 0.0],
        [min_x - 1.5, max_y + 0.2, 0.0],
        [min_x - 1.5, max_y - 0.4, 0.0],
    ]
...
def write_real_space_svg(path: Path, positions: list[list[float]], colors: list[str], bonds: list[tuple[int, int, float, str]]) -> None:
    ...
    for start_index, end_index, radius, color in bonds:
        ...
        x0, y0 = project(start)
        x1, y1 = project(end)
        stroke_width = 2.2 + 22.0 * float(radius)
        bond_lines.append(
    ...
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">
<rect x="0" y="0" width="{width}" height="{height}" fill="white" />
<text x="{width / 2:.2f}" y="30" text-anchor="middle" font-family="Consolas, monospace" font-size="22">Graphene bond-dimerization real-space patch</text>
<text x="{left}" y="{height - 20}" font-family="Consolas, monospace" font-size="14">Orange: strengthened bond   Blue/Red: A/B sublattices</text>
{''.join(bond_lines)}
{''.join(atom_circles)}
</svg>
"""
...
def analytic_gap(t: float, delta_t: float) -> float:
    strong_bond = t + delta_t
    return max(0.0, 2.0 * (strong_bond - 2.0 * t))


def numerical_gap(rows: list[tuple[float, float, float, float]]) -> float:
    if not rows:
        return 0.0
    return min(upper - lower for _, _, lower, upper in rows)


def write_dimerized_model_summary(
    path: Path,
    t: float,
    delta_t: float,
    b1: np.ndarray,
    ...
    strong_bond = t + delta_t
    if analytical_gap_value > 1e-6:
        gap_statement = (
            "A direct band gap opens in this minimal bond-dimerization model.\n"
            "For positive hoppings, Eg = 2 * max(0, delta_t - t).\n"
            f"With the current parameters, Eg = {analytical_gap_value:.6f}."
        )
    else:
        gap_statement = (
    ...
    text = f"""Graphene bond-dimerization model
--------------------------------
...
t = {t:.6f}
delta_t = {delta_t:.6f}
strong dimer bond = {strong_bond:.6f}
...
Numerical minimum gap from the reciprocal-space grid = {numerical_gap_value:.6f}
Analytical gap formula value = {analytical_gap_value:.6f}
"""
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Graphene reciprocal-space comparison</title>
</head>
<body style="margin: 24px; font-family: Consolas, monospace; background: white; color: #111;">
  <h1 style="margin-bottom: 8px;">Pristine vs Dimerized Graphene Reciprocal Space</h1>
  <p style="margin-top: 0;">delta_t = {delta_t:.6f}. {gap_line}</p>
  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; align-items: start;">
    <div>
      <h2 style="margin-top: 0;">Pristine graphene</h2>
      <iframe src="pristine_reciprocal_space_interactive.html" title="Pristine graphene reciprocal-space view" style="width: 100%; height: 760px; border: 1px solid #d0d0d0; border-radius: 10px;"></iframe>
    </div>
    <div>
      <h2 style="margin-top: 0;">Bond-dimerized graphene</h2>
      <iframe src="reciprocal_space_interactive.html" title="Dimerized graphene reciprocal-space view" style="width: 100%; height: 760px; border: 1px solid #d0d0d0; border-radius: 10px;"></iframe>
    </div>
  </div>
</body>
</html>
...
def write_dimerized_report_html(
    path: Path,
    delta_t: float,
    analytical_gap_value: float,
    numerical_gap_value: float,
    ...
    comparison_block = (
        '<iframe src="reciprocal_space_comparison.html" title="Pristine versus dimerized reciprocal comparison" '
        'style="width: 100%; height: 860px; border: 1px solid #d0d0d0; border-radius: 10px;"></iframe>'
        if comparison_available
        else '<div style="padding: 24px; border: 1px solid #d0d0d0; border-radius: 10px; background: #fafafa;">'
             '<strong>Comparison view was not generated.</strong><br/>'
             'See <code>interactive_status.txt</code> for the dependency/runtime reason.'
             '</div>'
    )

    gap_text = (
        f"delta_t = {delta_t:.6f}. Analytical gap formula: Eg = 2 * max(0, delta_t - t). "
        f"Current analytical gap = {analytical_gap_value:.6f}, numerical grid gap = {numerical_gap_value:.6f}."
    )
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Graphene Bond-Dimerization Report</title>
</head>
<body style="margin: 24px; font-family: Consolas, monospace; background: white; color: #111;">
  <h1 style="margin-bottom: 8px;">Graphene Bond-Dimerization Report</h1>
  <p style="margin-top: 0;">One nearest-neighbor bond is strengthened from <code>t</code> to <code>t + delta_t</code>. {gap_text}</p>
  <h2>Interactive Real Space</h2>
  {real_space_block}
  <h2 style="margin-top: 28px;">Static Real Space</h2>
  <img src="real_space.svg" alt="Dimerized graphene real-space patch" style="max-width: 100%; border: 1px solid #d0d0d0; border-radius: 10px;" />
  <h2 style="margin-top: 28px;">Band Structure (G-K-M-G)</h2>
  <img src="band_structure.svg" alt="Dimerized graphene band structure" style="max-width: 100%; border: 1px solid #d0d0d0; border-radius: 10px;" />
  <h2 style="margin-top: 28px;">2D Reciprocal Space</h2>
  <img src="reciprocal_space_map.svg" alt="Dimerized graphene reciprocal-space band map" style="max-width: 100%; border: 1px solid #d0d0d0; border-radius: 10px;" />
...
def main() -> None:
    ...
    parser.add_argument("--delta-t", type=float, default=0.3)
    ...
    pristine_fn = lambda k_value: pristine_bands(k_value, args.t, deltas)
    dimerized_fn = lambda k_value: dimerized_bands(k_value, args.t, args.delta_t, deltas)

    distances, bands, tick_positions, tick_labels = compute_band_path(
        band_fn=dimerized_fn,
        k_path=k_path,
        samples_per_segment=args.samples_per_segment,
    )

    max_hopping = max(abs(args.t), abs(args.t + args.delta_t))
    energy_limit = 3.4 * max(max_hopping, 1.0)
    ...
    pristine_reciprocal_rows = compute_reciprocal_map(
        band_fn=pristine_fn,
        polygon=polygon,
        grid_n=args.map_grid,
    )
    ...
    pristine_reciprocal_html = out_dir / "pristine_reciprocal_space_interactive.html"
    comparison_html = out_dir / "reciprocal_space_comparison.html"
    ...
    write_band_svg(band_svg, "Graphene bond-dimerization band structure", distances, bands, tick_positions, tick_labels)
    write_dos_csv(dos_csv, energy_axis, dos)
    write_dos_svg(dos_svg, "Graphene bond-dimerization DOS", energy_axis, dos)
    write_reciprocal_csv(reciprocal_csv, reciprocal_rows)
    write_reciprocal_svg(
        reciprocal_svg,
        "Graphene bond-dimerization reciprocal-space band map",
        polygon,
        reciprocal_rows,
        k_path,
        energy_limit,
    ...
    if interactive_ok:
        try:
            export_atomic_scene_html(
                ...
                title="graphene bond-dimerization real-space patch",
        ...
        try:
            ...
            export_reciprocal_surfaces_html(
                ...
                title="graphene bond-dimerization reciprocal-space surfaces",
            ...
            status_lines.append("Dimerized reciprocal-space interactive HTML generated successfully.")
        except Exception as exc:
            status_lines.append(f"Dimerized reciprocal-space interactive HTML export failed: {type(exc).__name__}: {exc}")

        try:
            pristine_points = np.array([[row[0], row[1]] for row in pristine_reciprocal_rows], dtype=float)
            pristine_lower = np.array([row[2] for row in pristine_reciprocal_rows], dtype=float)
            pristine_upper = np.array([row[3] for row in pristine_reciprocal_rows], dtype=float)
            path_points = np.vstack([point for _, point in k_path])
            path_labels = [label for label, _ in k_path]
            export_reciprocal_surfaces_html(
                output_path=pristine_reciprocal_html,
                title="pristine graphene reciprocal-space surfaces",
                xy_points=pristine_points,
                lower_energies=pristine_lower,
                upper_energies=pristine_upper,
                polygon_vertices=polygon,
                k_path=path_points,
                k_labels=path_labels,
                energy_limit=energy_limit,
            )
            pristine_reciprocal_ok = True
            status_lines.append("Pristine reciprocal-space interactive HTML generated successfully.")
        except Exception as exc:
            status_lines.append(f"Pristine reciprocal-space interactive HTML export failed: {type(exc).__name__}: {exc}")

        if reciprocal_interactive_ok and pristine_reciprocal_ok:
            write_comparison_html(comparison_html, args.delta_t, analytical_gap_value)
            comparison_ok = True
            status_lines.append("Pristine-vs-dimerized reciprocal comparison HTML generated successfully.")
    else:
        status_lines.append(f"PyVista unavailable: {interactive_reason}")

    write_status_note(interactive_status, status_lines)
    write_dimerized_report_html(
        path=report_html,
        delta_t=args.delta_t,
        analytical_gap_value=analytical_gap_value,
        numerical_gap_value=numerical_gap_value,
        real_space_interactive=real_space_interactive_ok,
        reciprocal_interactive=reciprocal_interactive_ok,
        comparison_available=comparison_ok,
    )
    ...
    print(f"[pristine 3d]    {pristine_reciprocal_html}")
    print(f"[comparison]     {comparison_html}")
    print(f"[report]         {report_html}")
    print(f"[3d status]      {interactive_status}")
    print(f"Analytical gap:  {analytical_gap_value:.6f}")
    print(f"Numerical gap:   {numerical_gap_value:.6f}")
    print(f"Dimerized K gap: {np.ptp(dimerized_fn(k_point)):.6f}")
